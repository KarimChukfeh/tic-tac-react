...
import { decodeCheckersMoves, getInstanceContract, getPlayerProfileContract, ZERO_ADDRESS, resolvePlayerProfileAddress } from '../lib/checkers';

const HISTORY_LIMIT = 30;
const POLL_INTERVAL_MS = 8000;

export function useCheckersV2MatchHistory(factoryContract, runner, account, options = {}) {
    ...
    try {
      ...
      const allMatches = (await Promise.all(records.map(async (record) => {
        try {
          ...
          const moveHistory = decodeCheckersMoves(m.moves || '').map((move, index) => ({
            player: index % 2 === 0 ? 'Light' : 'Dark',
            move: move.notation,
            from: move.from,
            to: move.to,
          ...
          for (let ci = 0; ci < boardArr.length; ci++) {
            packedBoard |= BigInt(Number(boardArr[ci]) & 0xF) << BigInt(ci * 4);
          }
      ...
      console.groupCollapsed(`[useCheckersV2MatchHistory] Loaded ${allMatches.length} completed matches for ${account}`);
      ...
      console.error('[useCheckersV2MatchHistory] Error:', err);
    } finally {
      setLoading(false);
    }
