...
import { getInstanceContract, getPlayerProfileContract, ZERO_ADDRESS, resolvePlayerProfileAddress } from '../lib/checkers';
...
function buildInstanceActivity(instance, account, dismissedMatches, matchResults) {
  ...
  for (const result of matchResults) {
    if (!result) continue;
    const { roundIdx, matchIdx, m, fullMatch } = result;
    ...
    if (matchStatus === 1 || matchStatus === 2) {
      ...
      let isMyTurn = false;
      if (matchStatus === 1) {
        const currentTurn = fullMatch?.currentTurn?.toLowerCase();
        isMyTurn = currentTurn === acc;
      }
...
export const useCheckersV2PlayerActivity = (instanceContract, account, factoryContract, runner) => {
    ...
    try {
      ...
      if (factoryContract && runner) {
        try {
          ...
          console.warn('[useCheckersV2PlayerActivity] Profile lookup failed:', profileErr.message);
        }
        ...
        if (instanceMap.size === 0 || instanceContract == null) {
          try {
            ...
            console.warn('[useCheckersV2PlayerActivity] activeTournaments fallback failed:', factoryErr.message);
          }
        }
      }
        ...
        for (let roundIdx = 0; roundIdx <= instance.currentRound && roundIdx < totalRounds; roundIdx++) {
          ...
          for (let matchIdx = 0; matchIdx < matchCount; matchIdx++) {
            ...
            matchCallSpecs.push({
              ...
              params: [ethers.solidityPackedKeccak256(['uint8', 'uint8'], [roundIdx, matchIdx])],
            });
          }
        }
      }
      ...
      for (let index = 0; index < matchDescriptors.length; index++) {
        ...
        const matchCallResult = matchResultCalls[index * 2];
        const fullMatchCallResult = matchResultCalls[(index * 2) + 1];
        if (!matchCallResult?.success) continue;

        const entries = matchResultsByInstance.get(descriptor.address) || [];
        entries.push({
          roundIdx: descriptor.roundIdx,
          matchIdx: descriptor.matchIdx,
          m: matchCallResult.result,
          fullMatch: fullMatchCallResult?.success ? fullMatchCallResult.result : null,
        });
      ...
      console.error('[useCheckersV2PlayerActivity] Error:', err);
