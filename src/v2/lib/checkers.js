...
import CheckersFactoryABIData from '../ABIs/CheckersFactory-ABI.json';
import LocalhostFactoryData from '../ABIs/localhost-checkers-factory.json';
...
export const PLAYER_PROFILE_ABI = getPlayerProfileAbi(CheckersFactoryABIData, PlayerProfileABIData);
export const PLAYER_REGISTRY_ABI = getPlayerRegistryAbi(CheckersFactoryABIData, PlayerRegistryABIData);
export const PLAYER_REGISTRY_ADDRESS = getPlayerRegistryAddress(CheckersFactoryABIData, PlayerRegistryABIData, 'CheckersFactory');

export const CHECKERS_V2_FACTORY_ADDRESS = getFactoryAddress(CheckersFactoryABIData);
export const CHECKERS_V2_FACTORY_ABI = getFactoryAbi(CheckersFactoryABIData);
export const CHECKERS_V2_INSTANCE_ABI = getInstanceAbi(CheckersFactoryABIData);
export const CHECKERS_V2_IMPLEMENTATION_ADDRESS = getImplementationAddress(CheckersFactoryABIData);
export const CHECKERS_V2_FACTORY_ADDRESS_CANDIDATES = getFactoryAddressCandidates({
  gameAbiData: CheckersFactoryABIData,
  localhostFactoryData: LocalhostFactoryData,
  hardhatFactoryData: HardhatFactoryData,
  etourFactoryAbis: ETourFactoryABIs,
  factoryName: 'CheckersFactory',
});
...
const CHECKERS_MOVE_PATTERN = /^\s*(\d{1,2}[-x]\d{1,2}K?)(?:\s*,\s*\d{1,2}[-x]\d{1,2}K?)*\s*$/i;
...
export function getFactoryContract(runner, address = CHECKERS_V2_FACTORY_ADDRESS) {
  return new ethers.Contract(address, CHECKERS_V2_FACTORY_ABI, runner);
}

export function getInstanceContract(address, runner) {
  return new ethers.Contract(address, CHECKERS_V2_INSTANCE_ABI, runner);
}
...
export function decodeCheckersMoves(movesString) {
  if (!movesString) return [];

  if (CHECKERS_MOVE_PATTERN.test(movesString)) {
    return movesString
      .split(',')
      .map((value) => value.trim())
      .filter(Boolean)
      .map((value) => {
        const match = value.match(/^(\d{1,2})([-x])(\d{1,2})(K?)$/i);
...
export function normalizeMatch(roundNumber, matchNumber, matchData, board) {
  ...
  const decodedMoves = decodeCheckersMoves(matchData.moves || '');
  return {
    roundNumber,
    matchNumber,
...
export function extractInstanceAddressFromReceipt(receipt) {
  const iface = new ethers.Interface(CHECKERS_V2_FACTORY_ABI);
