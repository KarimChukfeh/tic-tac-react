...
const CommunityRaffleCard = ({ raffleInfo, playerActivityHeight, onRefresh, onTriggerRaffle, syncing }) => {
  ...
  // PlayerActivity collapsed button size: ~48px mobile, ~64px desktop (with padding)
  // Spacing between components: 32px
  const BASE_TOP_MOBILE = 16; // top-4 in pixels
  const BASE_TOP_DESKTOP = 80; // md:top-20 in pixels
  const COLLAPSED_BUTTON_HEIGHT_MOBILE = 48; // collapsed button height on mobile
    ...
    >
      ...
      {!isExpanded && (
        <button
          ...
          className={`rounded-full p-3 md:p-4 border-2 transition-all hover:scale-110 shadow-xl relative group ${
            isFull
        ...
        >
          ...
          {/* Full Badge */}
          {isFull && (
            <div className="absolute -top-1 -right-1 bg-green-500 rounded-full w-6 h-6 flex items-center justify-center animate-pulse">
