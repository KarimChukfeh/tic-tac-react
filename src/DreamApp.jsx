  ...
  return (
    <GlassPanel style={{ marginBottom: '40px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      {gameLog && gameLog.length > 0 && (
        <GlassPanel style={{ marginTop: '30px', maxHeight: '280px', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
            <History style={{ color: '#8000ff' }} size={20} />
            <h3 style={{ fontSize: '18px', fontWeight: 'bold', color: '#8000ff' }}>Activity Log</h3>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto', maxHeight: '200px', paddingRight: '8px' }}>
            {gameLog.slice().reverse().map((log, idx) => (
              <div key={idx} style={{
                ...
                borderLeft: `3px solid ${
                  log.type === 'win' ? '#00ff88' :
                  log.type === 'move' ? '#00ffff' :
                  log.type === 'join' ? '#8000ff' :
                  'rgba(200, 200, 200, 0.5)'
                }`,
                background: `${
                  log.type === 'win' ? 'rgba(0, 255, 136, 0.1)' :
                  log.type === 'move' ? 'rgba(0, 255, 255, 0.1)' :
                  log.type === 'join' ? 'rgba(128, 0, 255, 0.1)' :
                  'rgba(100, 100, 100, 0.1)'
                }`
              }}>
                <div style={{ display: 'flex', alignItems: 'start', gap: '8px', flexWrap: 'wrap' }}>
                  <span style={{ color: 'rgba(200, 200, 200, 0.7)', fontSize: '11px', minWidth: 'fit-content' }}>{log.timestamp}</span>
                  <span style={{ color: 'rgba(255, 255, 255, 0.9)', fontSize: '12px', flex: 1 }}>{log.message}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <GlassPanel style={{ marginTop: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Clock size={16} style={{ color: '#00ffff' }} />
            <div>
              <div style={{ fontSize: '14px', color: '#00ffff', fontWeight: 'medium' }}>Auto-Sync</div>
              <div style={{ fontSize: '11px', color: 'rgba(0, 255, 255, 0.7)' }}>Polling blockchain</div>
            </div>
          </div>
          <div style={{ position: 'relative', width: '48px', height: '48px' }}>
            <svg style={{ transform: 'rotate(-90deg)', width: '48px', height: '48px' }}>
              <circle cx="24" cy="24" r="20" stroke="currentColor" strokeWidth="3" fill="none" style={{ color: 'rgba(0, 255, 255, 0.3)' }} />
              <circle
                cx="24" cy="24" r="20" stroke="currentColor" strokeWidth="3" fill="none"
                strokeDasharray={`${2 * Math.PI * 20}`}
                strokeDashoffset={`${2 * Math.PI * 20 * (1 - refreshProgress / 100)}`}
                style={{ color: '#00ffff', transition: 'all 0.075s linear' }}
                strokeLinecap="round"
              />
            </svg>
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontSize: '14px', fontWeight: 'bold', color: '#00ffff' }}>
    ...
    try {
      ...
      if (code === '0x' || code === '0x0') {
        ...
        throw new Error(
          ...
          `3. If wrong address, update CONTRACT_ADDRESS in DreamApp.jsx line 1027`
        );
      }
      ...
      try {
        ...
        if (Number(totalGames) > 0) {
          ...
          setLastGame({
            ...
            result: Number(latestGame.result),
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '80px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px', maxWidth: '1000px', margin: '0 auto 40px' }}>
            <GlassPanel style={{ background: 'rgba(0, 255, 136, 0.1)', border: '1px solid rgba(0, 255, 136, 0.3)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                <Trophy style={{ color: '#00ff88' }} size={20} />
                <span style={{ fontWeight: 'bold', color: '#00ff88' }}>Winner Takes 95%</span>
              </div>
              <p style={{ fontSize: '14px', color: '#00ff88', opacity: 0.8 }}>Champion walks away with almost the entire pot</p>
            </GlassPanel>
            <GlassPanel style={{ background: 'rgba(255, 215, 0, 0.1)', border: '1px solid rgba(255, 215, 0, 0.3)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                <DollarSign style={{ color: '#ffd700' }} size={20} />
                <span style={{ fontWeight: 'bold', color: '#ffd700' }}>0.002 ETH Entry</span>
              </div>
              <p style={{ fontSize: '14px', color: '#ffd700', opacity: 0.8 }}>Low stakes, high strategy gameplay</p>
            </GlassPanel>
            <GlassPanel style={{ background: 'rgba(0, 255, 255, 0.1)', border: '1px solid rgba(0, 255, 255, 0.3)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                <Zap style={{ color: '#00ffff' }} size={20} />
                <span style={{ fontWeight: 'bold', color: '#00ffff' }}>Random First Move</span>
              </div>
              <p style={{ fontSize: '14px', color: '#00ffff', opacity: 0.8 }}>On-chain coin flip decides who starts</p>
            </GlassPanel>
          </div>
          ...
          <div style={{ marginTop: '30px', maxWidth: '800px', margin: '30px auto 0' }}>
            <GlassPanel style={{ background: 'rgba(0, 100, 255, 0.1)', border: '1px solid rgba(0, 100, 255, 0.3)' }}>
              <div style={{ display: 'flex', alignItems: 'start', gap: '12px' }}>
                <Info size={18} style={{ color: '#00ffff', marginTop: '2px', flexShrink: 0 }} />
                <div style={{ fontSize: '14px' }}>
                  <p style={{ color: '#00ffff', fontWeight: 'bold', marginBottom: '8px' }}>Why Arbitrum?</p>
                  <p style={{ color: 'rgba(0, 255, 255, 0.8)', lineHeight: '1.6' }}>
                    This game runs on <a href="https://arbitrum.io" target="_blank" rel="noopener noreferrer" style={{ fontWeight: 'bold', color: '#00ffff', textDecoration: 'underline' }}>Arbitrum One</a>, an Ethereum Layer 2 network.
                    If prompted, simply click to switch networks in MetaMask — it's instant, free, and uses the same ETH you already have.
                    No bridging required. <span style={{ color: '#00ffff' }}>Lower fees, same security.</span>
                  </p>
                </div>
              </div>
            </GlassPanel>
          ...
          {account && (networkInfo || contractStatus !== 'not_checked') && (
            <div style={{ marginTop: '40px', maxWidth: '800px', margin: '40px auto 0' }}>
              <GlassPanel>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
                  <Info size={20} style={{ color: '#00ffff' }} />
                  <h3 style={{ fontSize: '18px', fontWeight: 'bold', color: '#fff' }}>Connection Status</h3>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '14px' }}>
                    ...
                    <div style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{
                          ...
                          height: '8px',
                          borderRadius: '50%',
                          background: networkInfo.isArbitrum ? '#00ff88' : '#ffd700'
                        }} />
                        <span style={{ color: 'rgba(200, 200, 200, 0.9)' }}>Network:</span>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{
                          fontWeight: 'bold',
                          color: networkInfo.isArbitrum ? '#00ff88' : '#ffd700'
                        }}>
                          {networkInfo.name}
                        </div>
                        <div style={{ fontSize: '11px', color: 'rgba(200, 200, 200, 0.7)' }}>Chain ID: {networkInfo.chainId}</div>
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <div style={{
                        ...
                        height: '8px',
                        borderRadius: '50%',
                        background: contractStatus === 'deployed' ? '#00ff88' :
                          contractStatus === 'checking' ? '#ffd700' :
                          contractStatus === 'not_deployed' ? '#ff0064' :
                          'rgba(200, 200, 200, 0.5)',
                        animation: contractStatus === 'checking' ? 'pulse 2s ease-in-out infinite' : 'none'
                      }} />
                      <span style={{ color: 'rgba(200, 200, 200, 0.9)' }}>Contract:</span>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{
                        fontWeight: 'bold',
                        color: contractStatus === 'deployed' ? '#00ff88' :
                          contractStatus === 'checking' ? '#ffd700' :
                          contractStatus === 'not_deployed' ? '#ff0064' :
                      ...
                      <div style={{ fontSize: '11px', color: 'rgba(200, 200, 200, 0.7)', fontFamily: 'monospace' }}>{shortenAddress(CONTRACT_ADDRESS)}</div>
                    </div>
                  </div>

                  {/* Deployment Instructions for Not Deployed */}
                  {contractStatus === 'not_deployed' && (
                    <div style={{
                      ...
                      <div style={{ color: '#ff0064', fontWeight: 'bold', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <AlertCircle size={16} />
                        Contract Not Found at This Address
                      </div>
                      <div style={{ fontSize: '11px', color: 'rgba(255, 100, 150, 0.9)', marginBottom: '12px' }}>
                        <div style={{ background: 'rgba(0, 0, 0, 0.3)', padding: '8px', borderRadius: '6px', fontFamily: 'monospace', fontSize: '10px', marginBottom: '12px' }}>
                          Checking: {CONTRACT_ADDRESS}
                        </div>
                        <div style={{ marginBottom: '8px', fontWeight: 'bold', color: '#ff0064' }}>🔍 Troubleshooting Steps:</div>
                        <div style={{ marginLeft: '16px', marginBottom: '8px' }}>
                          <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>1️⃣ Check Deployment Output</div>
                          <div style={{ fontSize: '10px', opacity: 0.9, marginLeft: '16px' }}>
                            Look for: <code style={{ background: 'rgba(0, 0, 0, 0.5)', padding: '2px 4px', borderRadius: '3px' }}>"Contract deployed to: 0x..."</code>
                          </div>
                        </div>
                        <div style={{ marginLeft: '16px', marginBottom: '8px' }}>
                          <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>2️⃣ Update the Address</div>
                          <div style={{ fontSize: '10px', opacity: 0.9, marginLeft: '16px' }}>
                            Update in: <code style={{ background: 'rgba(0, 0, 0, 0.5)', padding: '2px 4px', borderRadius: '3px', color: '#ffd700' }}>src/DreamApp.jsx line 1092</code>
                          </div>
                        </div>
                        <div style={{ marginLeft: '16px' }}>
                          <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>3️⃣ Verify on Arbiscan</div>
                          <div style={{ fontSize: '10px', opacity: 0.9, marginLeft: '16px' }}>
                            <code style={{ background: 'rgba(0, 0, 0, 0.5)', padding: '2px 4px', borderRadius: '3px', display: 'block', marginBottom: '4px' }}>
                  ...
                  {networkInfo && !networkInfo.isArbitrum && (
                    <div style={{
                      background: 'rgba(255, 215, 0, 0.1)',
                      border: '1px solid rgba(255, 215, 0, 0.3)',
                      borderRadius: '10px',
                      ...
                      <div style={{ color: '#ffd700', fontWeight: 'bold', marginBottom: '12px' }}>⚠️ Wrong Network</div>
                      <div style={{ fontSize: '12px', color: 'rgba(255, 215, 0, 0.9)', marginBottom: '12px' }}>
                        You're on <span style={{ fontWeight: 'bold' }}>{networkInfo.name}</span>. Switch to Arbitrum One network (Chain ID: 42161).
                      </div>
                      <button
        ...
        {totalGamesPlayed > 0 && (
          <GlassPanel style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
              <Trophy style={{ color: '#8000ff' }} size={24} />
