...
export function getFactoryAddress(abiData) {
  return abiData?.factory?.address ?? null;
}

export function getFactoryAbi(abiData) {
  return abiData?.factory?.abi ?? [];
}

export function getInstanceAbi(abiData) {
  return abiData?.instance?.abi ?? abiData?.instance?.instanceAbi ?? [];
}

export function getImplementationAddress(abiData) {
  return abiData?.instance?.address ?? abiData?.implementation?.address ?? null;
}

export function getPlayerProfileAbi(gameAbiData, fallbackAbiData) {
  return gameAbiData?.playerProfile?.PlayerProfileImpl?.abi ?? fallbackAbiData?.contract?.abi ?? [];
}

export function getPlayerRegistryAbi(gameAbiData, fallbackAbiData) {
  return gameAbiData?.playerProfile?.PlayerRegistry?.abi ?? fallbackAbiData?.contract?.abi ?? [];
}
