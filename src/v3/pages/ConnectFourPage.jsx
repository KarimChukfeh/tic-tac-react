...
export default function ConnectFourPage({ routeBase = '/v3/connect4' }) {
  ...
  const [arenaEffectsEnabled, setArenaEffectsEnabled] = useState(() => (
    getInitialArenaEffectsPreference(CONNECTFOUR_ARENA_EFFECTS_STORAGE_KEY)
  ));
  ...
  const resolveFactoryContract = async () => {
    ...
    throw new Error(`No validated Connect Four V3 factory found at ${CONNECTFOUR_FACTORY_ADDRESS_CANDIDATES.join(' or ')} on ${V3_NETWORK_NAME}.`);
  };
  ...
  const createInstance = async (event) => {
    ...
    try {
      ...
      console.error('[ConnectFourV3 createInstance] raw error:', error);
  ...
  const handleEnroll = useCallback(async () => {
    ...
    try {
      ...
      console.error('[ConnectFourV3] Enroll error:', error);
                  ...
                  <form onSubmit={createInstance}>
                    <div className="bg-slate-900/50 border border-purple-400/20 rounded-2xl p-4 md:p-5 t2-create-panel">
                      <div className="mb-4 flex items-center justify-between gap-3">
                        ...
                        {!account ? (
                          <button
                            ...
                            aria-controls="configure-lobby-panel-connectfour"
                      ...
                      {shouldRenderCreateFormBody ? (
                        <div
                          id="configure-lobby-panel-connectfour"
